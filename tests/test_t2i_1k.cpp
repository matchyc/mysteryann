...
class IndexBipartiteTest : public ::testing::Test {
   protected:
    ...
    uint32_t total_pts_;
    uint32_t n_q_;
    std::string base_file = "/ann/dataset/text-to-image/base.1k.fbin";
    std::string sampled_query_file = "/ann/dataset/text-to-image/query.learn.1k.fbin";
    std::string query_file = "/ann/dataset/text-to-image/query.public.100.fbin";
    std::string gt_file = "data/t2i_1k_query_gt.bin";
};
...
TEST_F(IndexBipartiteTest, SaveLoadTest) {
    ...
    parameters_.Set<uint32_t>("num_threads", 8);
    parameters_.Set<uint32_t>("M_sq", 10);
    parameters_.Set<uint32_t>("M_bp", 10);
    parameters_.Set<uint32_t>("L_pq", 50);
    parameters_.Set<uint32_t>("M_pjbp", 6);
    parameters_.Set<uint32_t>("L_pjpq", 100);
...
TEST_F(IndexBipartiteTest, SearchBipartiteTest) {
    ...
    std::vector<uint32_t> cmps_vec(q_pts);
    ...
    for (size_t i = 0; i < q_pts; ++i) {
        cmps_vec[i] = index.SearchBipartiteGraph(aligned_query_data + i * q_dim, k, i, parameters_, res);
    ...
    GTEST_COUT << "Average comparisons: " << avg_cmps / (float)q_pts << std::endl;
    // EXPECT_EQ(res.size(), q_pts);
}

TEST_F(IndexBipartiteTest, SearchProjectionTest) {
    parameters_.Set<uint32_t>("L_pq", 35);
    parameters_.Set<uint32_t>("num_threads", 1);
    ...
    for (size_t i = 0; i < q_pts; ++i) {
        ...
        visited.reset();
        // cmps_vec[i] = index.SearchProjectionGraph(aligned_query_data + i * q_dim, k, i, parameters_, res);
        // EXPECT_EQ(indices.size(), k);
        // for (auto &idx : indices) {
        //     EXPECT_LT(idx, nd_);
        // }
