...
int main(int argc, char** argv) {
    ...
    if (metric == mysteryann::COSINE) {
        ...
        for (size_t i = 0; i < query_num; ++i) {
            mysteryann::normalize<float>(query_load + i * dim, dim);
        }
    }
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        std::cout << "search time: " << diff << "\tAvg latency: " << (double)(total_latency) / (double)(query_num)
                  << "ms" << "\tcmps: " << avg_cmps
                  << "\tQSP: " << (float)query_num / (diff / 1000.0) << "\trecall@" << K << " "<< recall << "\trderr" << rderr << "\n";
        evaluation_ofs << L << "," << (float)query_num / (diff / 1000.0) << "," << avg_cmps
                       << "," << (double)(total_latency) / (double)(query_num) << "," << recall << "," << rderr << std::endl;
        index.total_cmps = 0;
