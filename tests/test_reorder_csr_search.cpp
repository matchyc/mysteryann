...
int main(int argc, char **argv) {
    ...
    mysteryann::IndexBipartite index(q_dim, base_num, dist_metric, nullptr);
    index.LoadProjectionGraph(projection_index_save_file.c_str());
    std::cout << "begin reorder" << std::endl;
    // index.new_order_ = index.gorder(5);
    std::ifstream reorder_in;
    reorder_in.open("reorder_10M.txt", std::ios::in);
    index.new_order_.clear();
    uint32_t temp;
    while (reorder_in >> temp) {
        index.new_order_.push_back(temp);
    }
    reorder_in.close();

    std::ifstream reorder_in_origin;
    reorder_in_origin.open("reorder_origin_10M.txt", std::ios::in);
    index.Porigin_.clear();
    while (reorder_in_origin >> temp) {
        index.Porigin_.push_back(temp);
    }
    reorder_in_origin.close();

    // save index.Porigin_ too
    // std::ofstream reorder_out_origin;
    // reorder_out_origin.open("reorder_origin_10M.txt", std::ios::out);
    // for (uint32_t i = 0; i < index.Porigin_.size(); i++) {
    //     reorder_out_origin << index.Porigin_[i] << std::endl;
    // }
    // reorder_out_origin.close();

    std::cout << "end reorder" << std::endl;
    std::cout << "begin reorder graph" << std::endl;
    index.ReorderAdjList(index.new_order_);
    std::cout << "end reorder graph" << std::endl;
    std::cout << "begin transform to csr" << std::endl;
    // index.ConvertAdjList2CSR(index.new_order_, index.row_ptr_, index.col_idx_);
    index.ConvertAdjList2CSR(index.row_ptr_, index.col_idx_, index.new_order_);
    // write index.new_order_ to file
    // std::ofstream reorder_out;
    // reorder_out.open("reorder_10M.txt", std::ios::out);
    // for (uint32_t i = 0; i < index.new_order_.size(); i++) {
    //     reorder_out << index.new_order_[i] << std::endl;
    // }
    // reorder_out.close();
    ...
    std::cout << "end transform to csr" << std::endl;
    index.LoadSearchNeededDataReorder(base_data_file.c_str(), sampled_query_data_file.c_str());
...
#pragma omp parallel for schedule(dynamic, 1)
        for (size_t i = 0; i < q_pts; ++i) {
            ...
            projection_cmps_vec[i] = index.SearchProjectionCSRwithOrder(aligned_query_data + i * q_dim, k, i, parameters, res + i * k, res_dists[i]);
