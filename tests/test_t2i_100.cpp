...
class IndexBipartiteTest : public ::testing::Test {
   protected:
    void SetUp() override {
        ...
        uint32_t dimension = 0;  // Align dimension to 8
        uint32_t n_sq = 0;
        uint32_t n_bp = 0;
        uint32_t n_q = 0;

        float *sq_data = nullptr;
        float *bp_data = nullptr;
        float *query_data = nullptr;

        // load from file
        load_meta<float>(base_file.c_str(), n_bp, dimension);
        load_meta<float>(sampled_query_file.c_str(), n_sq, dimension);
        load_meta<float>(query_file.c_str(), n_q, dimension);

        load_data<float>(base_file.c_str(), n_bp, dimension, bp_data);
        load_data<float>(sampled_query_file.c_str(), n_sq, dimension, sq_data);
        load_data<float>(query_file.c_str(), n_q, dimension, query_data);
        ...
        // compute closest neighbor for query_data under inner product metric
        ...
        for (size_t i = 0; i < n_q; i++) {
            for (size_t j = 0; j < n_bp; j++) {
                ...
                tmp_dist = -tmp_dist;  // mips, negtive to simulate
                if (tmp_dist < dist[i]) {
                    dist[i] = tmp_dist;
                    idx[i] = j;
                }
        ...
        // std::ofstream bp_file(base_file.c_str(), std::ios::binary);
        ...
        // bp_file.close();
        // std::ofstream sq_file(sampled_query_file.c_str(), std::ios::binary);
        ...
        // sq_file.close();
        // std::ofstream q_file(query_file.c_str(), std::ios::binary);
        ...
        uint32_t gt_dim = 1;
        std::ofstream gt_ofs(gt_file.c_str(), std::ios::binary);
        gt_ofs.write((char *)&(n_q), sizeof(uint32_t));
        gt_ofs.write((char *)&(gt_dim), sizeof(uint32_t));
        gt_ofs.write((char *)idx, n_q * gt_dim * sizeof(unsigned));
        gt_ofs.close();

        uint32_t *simul_res = new uint32_t[n_q];
        for (size_t i = 0; i < n_q; i++) {
            simul_res[i] = idx[i];
        }
        GTEST_COUT << "Simulate recall (should be 1.0): " << ComputeRecall(n_q, 1, 1, simul_res, idx) << std::endl;
        delete[] sq_data;
        delete[] bp_data;
        delete[] query_data;
        delete[] dist;
    ...
    uint32_t total_pts_;
    uint32_t n_q_;
    std::string base_file = "/ann/dataset/text-to-image/base.100.fbin";
    std::string sampled_query_file = "/ann/dataset/text-to-image/query.learn.100.fbin";
    std::string query_file = "/ann/dataset/text-to-image/query.public.100.fbin";
    std::string gt_file = "data/t2i_100_query_gt.bin";
};

TEST_F(IndexBipartiteTest, SaveLoadTest) {
    // Save bipartite graph
    std::string bipartite_file = "data/bipartite.bin";
    std::string projection_file = "data/projection.bin";

    parameters_.Set<uint32_t>("num_threads", 1);
    parameters_.Set<uint32_t>("M_sq", 10);
    parameters_.Set<uint32_t>("M_bp", 10);
    parameters_.Set<uint32_t>("L_pq", 10);
    parameters_.Set<uint32_t>("M_pjbp", 20);
    parameters_.Set<uint32_t>("L_pjpq", 20);
    omp_set_num_threads(parameters_.Get<uint32_t>("num_threads"));

    mysteryann::IndexBipartite index(dimension_, nd_, mysteryann::INNER_PRODUCT, nullptr);
    ...
    IndexBipartite index2(dimension_, nd_, mysteryann::INNER_PRODUCT, nullptr);
    index2.Load(bipartite_file.c_str());
    index2.LoadProjectionGraph(projection_file.c_str());
    ...
    for (size_t i = 0; i < nd_; ++i) {
        auto &n1 = index.GetProjectionGraph()[i];
        auto &n2 = index2.GetProjectionGraph()[i];
        EXPECT_EQ(n1.size(), n2.size());
        for (size_t j = 0; j < n1.size(); ++j) {
            EXPECT_EQ(n1[j], n2[j]);
        }
...
TEST_F(IndexBipartiteTest, SearchBipartiteTest) {
    ...
    // parameters_.Set<uint32_t>("M_sq", 10);
    // parameters_.Set<uint32_t>("M_bp", 10);
    parameters_.Set<uint32_t>("L_pq", 1);
    parameters_.Set<uint32_t>("num_threads", 1);
    parameters_.Set<uint32_t>("L_pjpq", 20);
    ...
    for (size_t i = 0; i < q_pts; ++i) {
        // index.Search(aligned_query_data + i * q_dim, nullptr, k, parameters_, indices.data());
        index.SearchBipartiteGraph(aligned_query_data + i * q_dim, k, i, parameters_, res);
...
TEST_F(IndexBipartiteTest, SearchProjectionTest) {
    ...
    uint32_t k = 1;
    uint32_t *res = new uint32_t[q_pts * k];
        boost::dynamic_bitset<> visited(nd_);
    // record the search time
    auto start = std::chrono::high_resolution_clock::now();
    // index.dist_cmp_metric.reset();
    // index.memory_access_metric.reset();
    for (size_t i = 0; i < q_pts; ++i) {
        visited.reset();
        // index.SearchProjectionGraph(aligned_query_data + i * q_dim, k, i, parameters_, res);
        // EXPECT_EQ(indices.size(), k);
        // for (auto &idx : indices) {
        //     EXPECT_LT(idx, nd_);
        // }
