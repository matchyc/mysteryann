...
int main(int argc, char **argv) {
    ...
    try {
        ...
        desc.add_options()("bipartite_index_save_path", po::value<std::string>(&bipartite_index_save_file)->default_value(""),
        ...
        desc.add_options()("L_pjpq", po::value<uint32_t>(&L_pjpq)->default_value(500),
                           "Priority queue length for projection graph searching");

        desc.add_options()("L_pq", po::value<uint32_t>(&L_pq)->default_value(500),
        ...
        desc.add_options()("num_threads,T", po::value<uint32_t>(&num_threads)->default_value(omp_get_num_procs()),
                           ...
                           "omp_get_num_procs())");
        desc.add_options()("learn_base_nn_path", po::value<std::string>(&learn_base_nn_file)->default_value(""),
                           "Path of learn-base NN file");
        desc.add_options()("base_learn_nn_path", po::value<std::string>(&base_learn_nn_file)->default_value(""),
    ...
    } else if (dist == "ip_build") {
        dist_metric = mysteryann::IP_BUILD;
        std::cout << "Using inner product as distance metric" << std::endl;
    }
    else{
    ...
    mysteryann::load_data_build<float>(base_data_file.c_str(), base_num, base_dim, data_bp);
    mysteryann::load_data_build<float>(sampled_query_data_file.c_str(), sq_num, sq_dim, data_sq);
    aligned_data_bp = data_bp;
    aligned_data_sq = data_sq;

    // if (dist_metric == mysteryann::INNER_PRODUCT) {
    //     mysteryann::ip_normalize(aligned_data_bp, base_dim);
    //     dist_metric = mysteryann::L2;
    // }
    std::cout << "Index save path: " << projection_index_save_file << std::endl;
    mysteryann::IndexBipartite index_bipartite(base_dim, base_num + sq_num, dist_metric, nullptr);
    ...
    index_bipartite.BuildGraphInline(sq_num, aligned_data_sq, base_num, aligned_data_bp, parameters);
