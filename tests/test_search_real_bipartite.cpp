...
//warm up
        for (size_t i = 0; i < 100; ++i) {
            index.SearchProjectionGraph(aligned_query_data + i * q_dim, k, i, parameters, res + i * k, res_dists[i]);
        }
...
#pragma omp parallel for schedule(dynamic, 1)
        for (size_t i = 0; i < q_pts; ++i) {
            ...
            auto ret_val = index.SearchProjectionGraph(aligned_query_data + i * q_dim, k, i, parameters, res + i * k, res_dists[i]);
