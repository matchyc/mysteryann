...
void IndexNSG::load_projection_format_nn_graph(const char *filename) {
  std::ifstream in(filename, std::ios::binary);
  uint32_t k;
  in.read((char *)&ep_, sizeof(uint32_t));
  uint32_t npts;
  in.read((char *)&npts, sizeof(uint32_t));
  final_graph_.resize(npts);
  // final_graph_.reserve(npts);
  std::cout << "knn graph npts: " << npts << std::endl;
  // uint32_t kk = (k + 3) / 4 * 4;
  for (size_t i = 0; i < npts; ++i) {
    in.read((char *)&k, sizeof(uint32_t));
    final_graph_[i].resize(k);
    // final_graph_[i].reserve(kk);
    in.read((char *)final_graph_[i].data(), k * sizeof(uint32_t));
  }
  std::cout << "npts: " << npts << " " << "k-size: " << final_graph_[0].size() << std::endl;
}

// WORONG READING
// void IndexNSG::load_gt_as_nn_graph(const char *filename) {
//   std::ifstream in(filename, std::ios::binary);
//   uint32_t npts;
//   in.read((char *)&npts, sizeof(uint32_t));
//   uint32_t k;
...
//     in.read((char *)final_graph_[i].data(), k * sizeof(uint32_t));
//   }
//   if (in.tellg() != sizeof(uint32_t) * 2 + k * sizeof(uint32_t) * npts) {
//     std::cerr << "load gt as nn graph error" << std::endl;
//     throw std::exception();
//   }
//   in.close();
// }
...
void IndexNSG::Build(size_t n, const float *data, const Parameters &parameters) {
  ...
  // load_gt_as_nn_graph(nn_graph_path.c_str());
  // load_projection_format_nn_graph(nn_graph_path.c_str());
  std::cout << "here" << std::endl;
  data_ = data;
  init_graph(parameters);
  std::cout << "finish init graph" << std::endl;
...
void IndexNSG::Search(const float *query, const float *x, size_t K,
                      const Parameters &parameters, unsigned *indices, float* res_dists) {
  ...
  for (size_t i = 0; i < K; i++) {
    ...
    res_dists[i] = retset[i].distance;
  }
}

uint32_t IndexNSG::search(const float *query, const float *x, size_t K,
                      const Parameters &parameters, unsigned *indices, float* res_dists, uint32_t& rec_hops) {
  const unsigned L = parameters.Get<unsigned>("L_search");
  data_ = x;
  std::vector<Neighbor> retset(L + 1);
  std::vector<unsigned> init_ids(L);
...
void IndexNSG::BuildBipartite(size_t n_sq, const float *sq_data, size_t n_bp,
                                const float *bp_data,
