// Created by Meng Chen from Fudan University in September 2023. email: mengchen22@m.fudan.edu.cn,
// mengchen9909@gmail.com Copyright reserved.
...
void IndexBipartite::BuildBipartite(size_t n_sq, const float *sq_data, size_t n_bp, const float *bp_data,
    ...
    // qbaseNNbipartite(parameters, simple_graph);
    std::cout << std::endl;
...
void IndexBipartite::qbaseNNbipartite(const Parameters &parameters, SimpleNeighbor *simple_graph) {
    uint32_t M_pjbp = parameters.Get<uint32_t>("M_pjbp");
...
#pragma omp parallel for schedule(static, 100)
    for (uint32_t it_sq = 0; it_sq < u32_nd_sq_; ++it_sq) {
        ...
        if (sq % 1000 == 0) {
            std::cout << "\r" << (100.0 * sq) / u32_nd_sq_ << "% of save bipartite graph finish" << std::flush;
        }
    }
}

std::pair<uint32_t, uint32_t> IndexBipartite::SearchQbaseNNBipartiteGraph(const float *query, size_t k, size_t &qid, const Parameters &parameters,
    ...
    while (search_queue.has_unexpanded_node()) {
        ...
        // std::cout << "qid" << qid << " cur_id: " << cur_id << std::endl;
...
#pragma omp parallel for schedule(static, 100)
    for (uint32_t it_sq = 0; it_sq < u32_nd_sq_; ++it_sq) {
        ...
        for (size_t i = 0; i < nn_base.size(); ++i) {
            ...
            float distance = distance_->compare(data_bp_ + dimension_ * (uint64_t)nn_base[i],
                                                data_bp_ + dimension_ * (uint64_t)cur_tgt, (unsigned)dimension_);
...
#pragma omp parallel for schedule(static, 100)
    for (uint32_t i = 0; i < vis_order.size(); ++i) {
        ...
        if (projection_graph_[node].size() < M_pjbp) {
            // std::cout << "Warning: projection graph node " << node << " has less than M_pjbp neighbors." <<
            // std::endl;
...
void IndexBipartite::PruneBiSearchBaseGetBase(std::vector<Neighbor> &search_pool, const float *query, uint32_t tgt_base,
    ...
    //                 continue;
    //             }
    //             float distance = distance_->compare(data_bp_ + dimension_ * (uint64_t)b_node, query,
    //             (unsigned)dimension_); base_pool.push_back({b_node, distance, false}); base_id.push_back(b_node);
    //             // ++cnt;
    //         }
    //         // if (cnt > 100) {
    //         //     break;
...
std::pair<uint32_t, uint32_t> IndexBipartite::SearchProjectionGraph(const float *query, size_t k, size_t &qid,
