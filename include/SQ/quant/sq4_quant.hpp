...
#include <iostream>
/**
 * SQ4：对向量做标量量化，用四个二进制位量化 float 类型，4字节->0.5字节
 */
namespace glass {

    template<Metric metric, typename Reorderer = FP32Quantizer <metric>, int DIM = 0>
    struct SQ4Quantizer {
        using data_type = uint8_t;
        constexpr static int kAlign = 128;
        float mx = -HUGE_VALF, mi = HUGE_VALF, dif;
        int d, d_align; // d_align 按 kAlign 128 对齐
        int64_t code_size; // code_size 按 64 对齐
