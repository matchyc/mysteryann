#ifndef MYSTERYANN_DISTANCE_H
#define MYSTERYANN_DISTANCE_H
...
enum Metric { L2 = 0, INNER_PRODUCT = 1, FAST_L2 = 2, PQ = 3, COSINE = 4, IP_BUILD = 5 };
class Distance {
   public:
...
// #endif
        ...
        if (UNLIKELY(size >= 4)) {
            __m128 mx = _mm_load_ps(a);
            a += 4;
            __m128 my = _mm_load_ps(b);
            b += 4;
...
class DistanceInnerProductBuild : public Distance {
   public:
    static inline __m128 masked_read(int d, const float *x) {
        // assert(0 <= d && d < 4);
        __attribute__((__aligned__(16))) float buf[4] = {0, 0, 0, 0};
