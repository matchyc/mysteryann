...
#ifndef mysteryann_UTIL_H
#define mysteryann_UTIL_H
#include <algorithm>
#include <chrono>
#include <cstring>
#include <fstream>
...
void load_data(const char *filename, uint32_t &points_num, uint32_t &dim, T *&data) {
    ...
    data = (T*)memalign(DATA_ALIGN_FACTOR, pts * new_dim * sizeof(T));
    for (size_t i = 0; i < points_num; i++) {
        // in.seekg(8 + i * (dim + 1) * sizeof(T), std::ios::beg);
    ...
    // __m512 aa = _mm512_load_ps(data + 32);
    dim = new_dim;
    // cursor position
    std::ios::pos_type ss = in.tellg();
...
// template <typename T>
// void load_data_search(const char *filename, uint32_t &points_num, uint32_t &dim, T *&data) {
//     uint32_t search_align = 16;
//     uint32_t align_build = 0;
//     std::cout << "load data from file: " << filename << std::endl;
//     std::ifstream in(filename, std::ios::binary);
//     if (!in.is_open()) {
//         std::cout << "open file error" << std::endl;
...
//     uint64_t pts = static_cast<uint64_t>(points_num);
//     uint64_t d = static_cast<uint64_t>(dim);
//     uint64_t new_dim = (d + search_align - 1) / search_align * search_align;
//     // data = new T[pts * d];
//     // check T type

//     // data = (T*)memalign(DATA_ALIGN_FACTOR * 8, pts * new_dim * sizeof(T));
//     std::cout << "new_dim: " << new_dim << std::endl;
//     data = (T*)memalign(search_align * 4, pts * new_dim * sizeof(T));
//     for (size_t i = 0; i < points_num; i++) {
//         // in.seekg(8 + i * (dim + 1) * sizeof(T), std::ios::beg);
//         in.read((char *)(data + i * new_dim), d * sizeof(T));
//         memset(data + i * new_dim + d, 0, (new_dim - d) * sizeof(T));
...
template <typename T>
void load_data_build_with_order(const char *filename, uint32_t &points_num, uint32_t &dim, T *&data, std::vector<uint32_t>& order) {
    uint32_t search_align = 8;
    uint32_t align_build = 0;
    std::cout << "load data from file: " << filename << std::endl;
    std::ifstream in(filename, std::ios::binary);
    if (!in.is_open()) {
    ...
    data = (T*)memalign(search_align * 4, pts * new_dim * sizeof(T));
    for (size_t i = 0; i < points_num; i++) {
        // std::cout << "i:" << i << "\t order_i: " << order[i] << std::endl;
        // in.seekg(8 + i * (dim + 1) * sizeof(T), std::ios::beg);
        in.read((char *)(data + order[i] * new_dim), d * sizeof(T));
        memset(data + order[i] * new_dim + d, 0, (new_dim - d) * sizeof(T));
        // if ((i + 1) % 100000 == 0)
        //     std::cout << "i: " << i << std::endl;
    }
    // __m512 aa = _mm512_load_ps(data + 32);
...
template <typename T>
void load_data_search_with_order(const char *filename, uint32_t &points_num, uint32_t &dim, T *&data, std::vector<uint32_t>& order) {
    uint32_t search_align = 16;
    uint32_t align_build = 0;
    std::cout << "load data from file: " << filename << std::endl;
    std::ifstream in(filename, std::ios::binary);
...
template <typename T>
void load_data_build(const char *filename, uint32_t &points_num, uint32_t &dim, T *&data) {
    uint32_t align_build = 8;
    std::cout << "load data from file: " << filename << std::endl;
    std::ifstream in(filename, std::ios::binary);
    if (!in.is_open()) {
        std::cout << "open file error" << std::endl;
    ...
    uint64_t pts = static_cast<uint64_t>(points_num);
    uint64_t d = static_cast<uint64_t>(dim);
    uint64_t new_dim = (d + align_build - 1) / align_build * align_build;
    // data = new T[pts * d];
    // check T type

    // data = (T*)memalign(DATA_ALIGN_FACTOR * 8, pts * new_dim * sizeof(T));
    std::cout << "new_dim: " << new_dim << std::endl;
    data = (T*)memalign(align_build * 4, pts * new_dim * sizeof(T));
    for (size_t i = 0; i < points_num; i++) {
        // in.seekg(8 + i * (dim + 1) * sizeof(T), std::ios::beg);
        in.read((char *)(data + i * new_dim), d * sizeof(T));
        memset(data + i * new_dim + d, 0, (new_dim - d) * sizeof(T));
...
template <typename T>
void load_data_with_skip(const char *filename, size_t &points_num, size_t &dim, T *&data, size_t skip = 0) {
    std::cout << "load data from file: " << filename << "skip num: " << skip << std::endl;
    std::ifstream in(filename, std::ios::binary);
    if (!in.is_open()) {
        std::cout << "open file error" << std::endl;
        exit(-1);
    }
    in.seekg(sizeof(uint32_t) * 2 + sizeof(T) * dim * (size_t)skip, std::ios::beg);
    std::cout << "points_num: " << points_num << " dim: " << dim << std::endl;
    uint64_t pts = static_cast<uint64_t>(points_num);
    uint64_t d = static_cast<uint64_t>(dim);
    if (data != nullptr) {
    ...
    //     throw std::runtime_error("Data file size wrong!");
    // }
    std::cout << "load data from file: " << filename << " points_num: " << points_num << " dim: " << dim << " skip num: " << skip << std::endl;
    in.close();
}
